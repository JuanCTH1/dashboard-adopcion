...
export function ProgressiveHierarchy({
  ...
  const actionPlanData = useMemo(() => {
    ...
    if (gapOrders === 0 || currentPct >= TARGET_ADOPTION_PCT) {
      ...
      return {
        ...
        accumulatedGain: 0,
        targetReached: true,
        projectedPct: Number(currentPct.toFixed(1)),
        totalPedidos,
        currentDigitales
      };
    }
    ...
    const projectedDigitales = currentDigitales + accumulatedGain;
    const projectedPct = totalPedidos > 0 ? Math.min(100, Number(((projectedDigitales / totalPedidos) * 100).toFixed(1))) : 100;

    return {
      clients: prioritizedClients,
      allCandidatesCount: candidates.length,
      currentPct: Number(currentPct.toFixed(1)),
      targetPct: TARGET_ADOPTION_PCT,
      gapOrders,
  ...
  const actionableAccountsCount = actionPlanData.clients.length;
            ...
            <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              {rightPanelTab === 'action_plan' ? (
                <div className="overflow-y-auto overflow-x-hidden flex-1 min-h-0 scrollbar-thin space-y-2 pr-0.5">
                  {/* MOTIVATIONAL 85% TARGET BANNER */}
                  <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800/90 border border-border shadow-2xs flex flex-col gap-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-1.5 font-extrabold text-xs text-foreground">
                        <Target className="w-3.5 h-3.5 text-primary" />
                        <span>Path to 85% Target</span>
                      </div>
                      <div className="flex items-center gap-1 text-xs tabular-nums">
                        <span className="font-bold text-muted-foreground">Current:</span>
                        <span className="font-black text-foreground">{actionPlanData.currentPct}%</span>
                        <ArrowRight className="w-3 h-3 text-primary shrink-0" />
                        <span className="font-black text-emerald-600 dark:text-emerald-400">Target: 85%</span>
                      </div>
                    </div>

                    {actionPlanData.targetReached ? (
                      <div className="text-xs text-emerald-700 dark:text-emerald-400 font-bold flex items-center gap-1.5 pt-0.5">
                        <Check className="w-3.5 h-3.5 shrink-0" />
                        <span>85% Target reached in this scope! Showing top stretch accounts.</span>
                      </div>
                    ) : (
                      <div className="flex flex-col gap-1 text-xs">
                        <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden">
                          <div
                            ...
                            style={{ width: `${Math.min(100, (actionPlanData.currentPct / 85) * 100)}%` }}
                          />
                        </div>
                        <div className="flex items-center justify-between text-xs text-muted-foreground font-medium pt-0.5">
                          <span>Gap: <b className="text-foreground">{formatNumber(actionPlanData.gapOrders)}</b> orders/mo</span>
                          <span className="text-emerald-700 dark:text-emerald-300 font-bold">
                            Convert these <b>{actionPlanData.clients.length} key accounts</b> to reach <b>{actionPlanData.projectedPct}%</b>
                          </span>
                        </div>
                      </div>
                    )}
                  ...
                  {actionPlanData.clients.length === 0 ? (
                    ...
                    actionPlanData.clients.map((cli, idx) => {
