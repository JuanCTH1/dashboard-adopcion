...
import { ChevronRight, ArrowUpDown, Download, CheckCircle2, XCircle, PhoneCall, UserCheck, Layers } from 'lucide-react';
...
export function HierarchyTable({
  ...
  return (
    <Card className="p-5 bg-card border border-border shadow-xs rounded-xl flex flex-col relative overflow-hidden">
      {/* Barra superior */}
      <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-primary via-indigo-500 to-sky-400" />

      {/* Encabezado con Breadcrumbs y Exportación */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3.5 border-b border-border">
        <div>
          <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1 flex items-center gap-1.5">
            ...
            <span>Navegación Multinivel (Drill-Down)</span>
          </div>
      ...
      {/* Si estamos en Cartera de un Vendedor Concreto */}
      {carteraVendedor ? (
        <div className="mt-3.5 overflow-x-auto">
          <div className="text-xs font-bold text-foreground mb-2.5 flex items-center justify-between">
            ...
            <span className="text-[11px] text-muted-foreground font-medium">
              Ordenado por volumen potencial (Prioridad Pareto)
            </span>
          </div>
              ...
              {carteraVendedor.map(cli => (
                <tr key={cli.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/60 transition-colors">
                  <td className="py-2.5 px-3 font-bold font-mono text-foreground flex items-center gap-1.5">
                    {cli.esTopPareto && (
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-500" title="Cuenta Top Pareto (20% volumen)" />
                    )}
                    <span>{cli.id}</span>
                  </td>
        ...
        /* Tabla Principal de Nodos (VPs, Directores, Gerentes, Vendedores) */
        <div className="mt-3.5 overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-border text-[11px] font-bold text-muted-foreground bg-slate-100/70 dark:bg-slate-900/60">
                ...
                <th className="py-2.5 px-3 min-w-[190px] cursor-pointer select-none" onClick={() => toggleSort('adopcion')}>
                  <div className="flex items-center gap-1">
                    <span>% Adopción Digital (Meta 75%)</span>
                    <ArrowUpDown className="w-3 h-3" />
                  </div>
                </th>
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-muted-foreground font-medium">
                            {nodo.tipo} · {c?.asignados} cuentas en cartera
                          </div>
                        </div>
                      </div>
                    </td>
